...
<form method="post">
    ...
    <div class="row g-3">
        ...
        <div class="col-3">
            ...
            <input type="text" class="form-control <?php if(session('errors.amount')) : ?>is-invalid<?php endif ?>"
                id="amount" name="amount" value="<?=$invoice->amount ?>">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <select id="project_id" name="project_id" class="form-select ">
                <option value="0" <?=($invoice->project_id == 0)? 'selected' : ''?>>-- Bitte auswählen --</option>
        ...
        <div class="col-md-4">
            ...
            <select id="website_id" name="website_id" class="form-select ">
                <option value="0" <?=($invoice->website_id == 0)? 'selected' : ''?>>-- Bitte auswählen --</option>
        ...
        <div class="col-md-6">
            ...
            <select id="renew_interval" name="renew_interval" class="form-select">
                ...
                <option value="1" <?=($invoice->renew_interval == 1)? 'selected' : ''?>>Monatlich (Rechnungsdatum)
                </option>
                <option value="2" <?=($invoice->renew_interval == 2)? 'selected' : ''?>>Monatlich (1. im Monat)</option>
                <option value="3" <?=($invoice->renew_interval == 3)? 'selected' : ''?>>Jährlich (Rechnungsdatum)
                </option>
                <option value="4" <?=($invoice->renew_interval == 4)? 'selected' : ''?>>Jährlich (1. Januar)</option>
            </select>
        </div>
        <div class="col-md-6">
